class LR1Item:
    ...
    def __hash__(self):
        return hash((
            ...
            self.rhs,
            self.dot_pos,
            frozenset(self.lookaheads)
        ))

    def core_eq(self, other):
        """Checks whether LR(0) cores are identical"""
    ...
    def __repr__(self):
        ...
        rhs_with_dot = (
            rhs_str[:self.dot_pos] +
            ['•'] +
            rhs_str[self.dot_pos:]
        )

        lookaheads_str = "/".join(
            sorted(list(self.lookaheads))
        )
...
def closure(items, grammar, first_sets):
    """
    Computes closure of LR(1) items.
    """
                ...
                if B in grammar.non_terminals:
                    ...
                    for a in list(item.lookaheads):
                        ...
                        for gamma in grammar.productions.get(B, []):
                            ...
                            # Merge lookaheads if core exists
                            found = False
                            ...
                            for existing in closure_set:
                                ...
                                if existing.core_eq(new_item):
                                    ...
                                    if not new_item.lookaheads.issubset(
                                        existing.lookaheads
                                    ):
                                        existing.lookaheads.update(
                                            new_item.lookaheads
                                        )
...
def canonical_collection(grammar, first_sets):
    """
    Generates canonical LR(1) collection.
    """

    start_item = LR1Item(
        grammar.start_symbol,
        grammar.productions[grammar.start_symbol][0],
        0,
        {'$'}
    )
